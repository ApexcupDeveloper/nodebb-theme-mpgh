...
theme.init = function(params, callback) {
  ...
  params.router.get('/admin/plugins/mpgh', params.middleware.admin.buildHeader, renderAdmin);
  params.router.get('/api/admin/plugins/mpgh', renderAdmin);
...
theme.getThemeConfig = function(config, callback) {
  meta.settings.get('mpgh', function(err, settings) {
    if (err) {
      return callback(err);
    }
    config.hideSubCategories = settings.hideSubCategories === 'on';
    config.selectedSkin = themeMpghSkin || settings.skinOption || 'default';
...
theme.customSettings = function(data, callback) {
  ...
  availableSkins.forEach(function(skin) {
    skinSelected = (data.settings.themeMpghSkin === skin.value) ? 'selected' : '';
    options = options + '<option value="' + skin.value + '" ' + skinSelected + '>' + skin.name + '</option>';
  });

  data.customSettings.push({
    title: 'Select skin for theme',
    content: '<div class="form-group fg-line"><div class="select"><select data-property="themeMpghSkin" class="form-control"><option value="">None</option>' + options + '</select></div></div>'
  });
...
theme.saveSettings = function(data, callback) {
  db.setObjectField('user:' + data.uid + ':settings', 'themeMpghSkin', data.settings.themeMpghSkin, callback);
};

theme.getSettings = function(data, callback) {
  themeMpghSkin = data.settings.themeMpghSkin;
  callback(null, data);
};
